...
vi.mock('../transcription.js', () => ({
  isSlackAudioFile: vi.fn((file: { mimetype?: string }) => file.mimetype?.startsWith('audio/') ?? false),
  transcribeSlackAudioFile: vi.fn().mockResolvedValue('hello from slack audio'),
}));
...
import { transcribeSlackAudioFile } from '../transcription.js';
...
describe('SlackChannel', () => {
  ...
  describe('file handling', () => {
    it('transcribes audio attachments into message content', async () => {
      const opts = createTestOpts();
      const channel = new SlackChannel(opts);
      await connectChannel(channel);
        ...
        files: [
          {
            ...
            name: 'memo.ogg',
            mimetype: 'audio/ogg',
            url_private_download: 'https://files.slack.test/memo.ogg',
          },
        ],
      });

      expect(transcribeSlackAudioFile).toHaveBeenCalled();
      expect(opts.onMessage).toHaveBeenCalledWith(
        REGISTERED_JID,
        expect.objectContaining({
          content: expect.stringContaining('[Voice: hello from slack audio]'),
        }),
      );
    });

    it('falls back when audio transcription returns null', async () => {
      vi.mocked(transcribeSlackAudioFile).mockResolvedValueOnce(null);
    ...
    it('falls back when audio transcription throws', async () => {
      vi.mocked(transcribeSlackAudioFile).mockRejectedValueOnce(new Error('boom'));
      ...
      expect(opts.onMessage).toHaveBeenCalledWith(
        REGISTERED_JID,
        expect.objectContaining({
          content: expect.stringContaining('[Voice Message - transcription failed]'),
        }),
      );
    });
  });
