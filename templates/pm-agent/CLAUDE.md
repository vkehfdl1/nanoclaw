...
Drive issue-to-implementation execution with high signal and low latency. GitHub webhook events are the primary trigger. Scheduled polling is fallback reconciliation only.
...
1. `gh_pr_diff(repo="__REPO_ALIAS__", pr_number=<number>)`
2. Inspect mounted files under `/workspace/extra/__REPO_ALIAS__/`.
...
4. `codex_exec(repo="__REPO_ALIAS__", branch="", prompt="<prompt>")`
5. `gh_pr_review(repo="__REPO_ALIAS__", pr_number=<number>, review_event="<verdict>", body="<review body>")`
6. Post a concise Slack update with the verdict.

## SecondBrain

Write `pm-insight` entries with source `__GROUP_FOLDER__` and project `__REPO_ALIAS__` for each triaged issue and significant implementation or PR review outcome.
